...
namespace EasyFramework.Framework
{
    ...
    public class AutoBindGlobalSettingProvider : SettingsProvider
    {
        ...
        /// 自动绑定全局设置路径
        /// </summary>
        const string m_AutoBindGlobalSettingPath = "Assets/EasyFramework/Resources/Settings/AutoBindGlobalSetting.asset";
        
        /// <summary>
        /// 在设置中的标题名称
        /// </summary>
        private const string m_HeaderName = "EF/AutoBindGlobalSetting";
        ...
        {
            if (File.Exists(m_AutoBindGlobalSettingPath))
            {
                var provider = new AutoBindGlobalSettingProvider(m_HeaderName, SettingsScope.Project)
                {
                    keywords = GetSearchKeywordsFromGUIContentProperties<AutoBindGlobalSetting>()
                };
                return provider;
            }
            return null;
