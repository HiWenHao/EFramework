...
namespace EasyFramework.UnityWebSocket
{
    public class WebSocket : IWebSocket
    {
        ...
        public event Action<ConnectEventArgs> OnOpen;
        public event Action<DisconnectEventArgs> OnClose;
        ...
        private void HandleOpen()
        {
            ...
            HandleEventSync(new ConnectEventArgs());
#else
            OnOpen?.Invoke(new OpenEventArgs());
#endif
        }
...
#else
            OnMessage?.Invoke(new MessageEventArgs(opcode, rawData));
#endif
        }
...
#if !UNITY_WEB_SOCKET_ENABLE_ASYNC
            HandleEventSync(new DisconnectEventArgs(code, reason));
#else
            OnClose?.Invoke(new CloseEventArgs(code, reason));
#endif
        }
...
#else
            OnError?.Invoke(new ErrorEventArgs(exception.Message));
#endif
        }
        ...
        internal void Update()
        {
            ...
            while (eventQueue.Count > 0)
            {
                ...
                {
                    OnClose?.Invoke(e as DisconnectEventArgs);
                }
                else if (e is ConnectEventArgs)
                {
                    OnOpen?.Invoke(e as ConnectEventArgs);
                }
                else if (e is MessageEventArgs)
                {
                    OnMessage?.Invoke(e as MessageEventArgs);
                }
        ...
        {
            D.Log($"<color=yellow>[EasyFramework.UnityWebSocket]</color>" +
