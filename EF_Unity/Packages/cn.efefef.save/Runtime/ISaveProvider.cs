 ...
 *                （文件加密 / SQLite / 云端同步），SaveManager 不感知底层实现。
 * Author:        Alvin8412
...
namespace EasyFramework.Systems.Save
{
    ...
    /// <list type="bullet">
    /// <item><see cref="FileSaveProvider"/> — 默认：AES加密 + 本地文件</item>
    /// <item><b>SqliteSaveProvider</b> — 未来：SQLite数据库（支持条件查询、批量写入）</item>
    /// <item><b>CloudSaveProvider</b> — 未来：云端同步（HTTP上传/下载）</item>
    /// <item><b>EncryptedGzipProvider</b> — 未来：压缩+加密组合</item>
    /// </list>
    /// </para>
    /// <para>切换方式：<c>SaveManager.Instance.SetProvider(new YourProvider());</c></para>
    /// </summary>
    public interface ISaveProvider
    {
        /// <summary>
        /// 持久化一条数据。数据由 SaveManager 完成 JSON 序列化后传入原始字节。
