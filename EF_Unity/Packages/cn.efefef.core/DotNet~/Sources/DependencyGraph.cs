...
namespace EasyFramework
{
    internal class DependencyGraph
    {
        ...
        /// <summary>返回所有环的列表，每个环是字符串列表 [A, B, C, A]</summary>
        ...
        private bool FindCycle(string current, Dictionary<string, VisitState> state,
            ...
            if (_edges.TryGetValue(current, out var neighbors))
            {
                foreach (var neighbor in neighbors)
                {
                    ...
                    else if (neighborState == VisitState.Visiting)
                    {
                        ...
                        var cycle = new List<string>(path.Skip(idx));
                        cycle.Add(neighbor);
                        cycles.Add(cycle);
                    }
                }
